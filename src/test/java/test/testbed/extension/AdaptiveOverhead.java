...
public class AdaptiveOverhead extends ApplicationFrame {
	...
	private static final double[] adaptive = {10.004, 35.053 };

	private static final double[] ann = {

	5.07,30.091 };

	private static final double[] armax = { 0.101,0.205 };

	private static final double[] rt = { 0.4,0.4 };
	...
	/**
	 * Max of edu.rice.rubis.servlets.SearchItemsByCategory's
	 * Availability.rtf=1.0 Max of
	 * edu.rice.rubis.servlets.SearchItemsByCategory's Response
	 * Time.rtf=0.1761915641476274 Max of
	 * edu.rice.rubis.servlets.SearchItemsByCategory's
	 * Throughput.rtf=28.199060031332287
	 * 
	 * Response Time: Prediction RMAPE: 0.20471427118277635
	 * 
	 * 
	 * Throughput: Prediction RMAPE: 0.15320254706329603 Prediction RS:
	 * 0.9999946698839458
	 * 
	 * Availability: Prediction RMAPE: 0.010236337390917246 Prediction RS:
	 * 0.9999996056410796
	 */
	 ...
	 *            the frame title.
	 */
	public AdaptiveOverhead(final String title) {
		super(title);
	...
	private CategoryDataset createDataset() {
		...
		// row keys...
		final String series1 = "best";
		final String series2 = "worest";

		// column keys...
		/*
		 * final String category1 = "Response Time"; final String category2 =
		 * "Throughput"; final String category3 = "Availability";
		 */

		final String category1 = "Adaptive";
		final String category2 = "ANN";
		final String category3 = "ARMAX";
		final String category4 = "RT";

		// create the dataset...
		final DefaultCategoryDataset dataset = new DefaultCategoryDataset();


		dataset.addValue(adaptive[0] , series1, category1);
		dataset.addValue(adaptive[1] , series2, category1);
		dataset.addValue(ann[0] , series1, category2);
		dataset.addValue(ann[1] , series2, category2);
		dataset.addValue(armax[0] , series1, category3);
		dataset.addValue(armax[1] , series2, category3);
		dataset.addValue(rt[0] , series1, category4);
		dataset.addValue(rt[1] , series2, category4);
	...
	private void createData(XYSeries series1, XYSeries series2,
			XYSeries series3, XYSeries series4, XYSeries series5,
			XYSeries series6) {
	...
	private double calculate(double[] ideal, double[] actual, List<Double> ampes) {
		...
		for (int j = 0; j < ideal.length; j++) {
			...
			if ((actual[j] > 0 && ideal[j] < 0)
				...
				MAPE = Math
						.abs((ideal[j] + actual[j]) / (ideal[j] - actual[j]));
			} else {
				MAPE = Math
						.abs((ideal[j] - actual[j]) / (ideal[j] + actual[j]));
			}
	...
	private JFreeChart createChart(final CategoryDataset dataset) {
		...
		catRenderer.setBaseItemLabelGenerator(new NumberLabelGenerator("",
		...
		rangeAxis.setRange(0, 40);// 0.45, 0.8
		CategoryAxis domain = (CategoryAxis) plot.getDomainAxis();
		rangeAxis.setTickUnit(new NumberTickUnit(5));// 0.1, 0.1
		rangeAxis.setLabelFont(new java.awt.Font("Dialog", java.awt.Font.BOLD,
				12));
		domain.setLabelFont(new java.awt.Font("Dialog", java.awt.Font.BOLD, 12));

		renderer.setDrawBarOutline(false);
		renderer.setShadowVisible(false);
		renderer.setItemLabelFont(new java.awt.Font("Dialog",
		...
		final GradientPaint gp1 = new GradientPaint(0.0f, 0.0f, Color.red,
				0.0f, 0.0f, Color.red);
		final GradientPaint gp2 = new GradientPaint(0.0f, 0.0f, Color.magenta,
				0.0f, 0.0f, Color.magenta);

		final GradientPaint gp3 = new GradientPaint(0.0f, 0.0f, Color.orange,
				0.0f, 0.0f, Color.orange);

		final GradientPaint gp4 = new GradientPaint(0.0f, 0.0f, Color.cyan,
	...
	public static void main(final String[] args) {
		...
		final AdaptiveOverhead demo = new AdaptiveOverhead(title);
		demo.pack();
		RefineryUtilities.centerFrameOnScreen(demo);
		demo.setVisible(true);

		/*
		 * try { Thread.sleep((long)2000); } catch (InterruptedException e1) {
		 * // TODO Auto-generated catch block e1.printStackTrace(); }
		 * 
		 * File file = new File("/Users/tao/research/chart.png"); try {
		 * ChartUtilities.saveChartAsPNG(file, demo.chart, 380, 170); } catch
	...
	public static void main1(final String[] args) {
		...
		try {
			reader = new BufferedReader(new FileReader(
					"/Users/tao/research/monitor/rs"));
			String line = null;
			while ((line = reader.readLine()) != null) {
				if (!line.contains("MSE")) {
					actuals.add(Double.parseDouble(line.split(",")[0]
							.split("=")[1]));
	...
	public synchronized static boolean record(String name, double timeUsage,
		...
		recoder.add(new Record(name, timeUsage, String.valueOf(Integer
				.parseInt(numberOfThread) * division)));
	...
	static class NumberLabelGenerator extends
			AbstractCategoryItemLabelGenerator implements
			CategoryItemLabelGenerator {
		public NumberLabelGenerator(String labelFormat, NumberFormat formatter,
				NumberFormat percentFormatter) {
			super(labelFormat, formatter, percentFormatter);
		}

		protected NumberLabelGenerator(String labelFormat,
